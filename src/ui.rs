...
use egui::{collapsing_header::paint_default_icon, epaint::Shadow, *};

use crate::{Controller, InPin, InPinId, Node, NodeId, OutPin, OutPinId, Snarl};
...
impl<T> Snarl<T> {
        ...
        V: SnarlViewer<T>,
    {
        ...
        bg_frame.show(ui, |ui| {
            ...
            let controller = Controller(&mut snarl_state);
            viewer.begin(controller, self);
            ...
            if bg_r.clicked_by(PointerButton::Primary) {
                viewer.clicked(input.modifiers, Controller(&mut snarl_state));
            }

            // Wire end position will be overridden when link graph menu is opened.
            let wire_end_pos = input.hover_pos.unwrap_or_default();
        ...
        V: SnarlViewer<T>,
    {
        ...
        if r.clicked_by(PointerButton::Primary) {
            viewer.node_clicked(input.modifiers, node, Controller(snarl_state));
        }
